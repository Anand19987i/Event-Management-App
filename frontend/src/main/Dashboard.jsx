        ...
        <>
            ...
            <div className="min-h-screen bg-gray-100 p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    {/* Total Events */}
                    <div className="bg-indigo-500 text-white p-6 rounded-xl shadow-lg transform hover:scale-105 transition-all">
                        <div className="flex items-center gap-4">
                            <i className="fas fa-calendar-alt text-4xl"></i>
                            <span className="text-xl font-semibold">Total Events</span>
                        </div>
                        <p className="mt-4 text-3xl font-bold">{hostData?.hostedEvents?.length}</p>
                    </div>

                    {/* Tickets Sold */}
                    <div className="bg-gray-600 text-white p-6 rounded-xl shadow-lg transform hover:scale-105 transition-all">
                        <div className="flex items-center gap-4">
                            <i className="fa-solid fa-ticket text-4xl"></i>
                            <span className="text-xl font-semibold">Tickets Sold</span>
                        </div>
                        <p className="mt-4 text-3xl font-bold">{hostData?.allTimeTicketSold}</p>
                    </div>

                    {/* Total Revenue */}
                    <div className="bg-yellow-500 text-white p-6 rounded-xl shadow-lg transform hover:scale-105 transition-all">
                        <div className="flex items-center gap-4">
                            <i className="fa-solid fa-indian-rupee-sign text-4xl"></i>
                            <span className="text-xl font-semibold">Total Revenue</span>
                        </div>
                        <p className="mt-4 text-3xl font-bold">{hostData?.allTimeRevenue}</p>
                    </div>
                </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-gray-200 text-gray-700">
                                    ...
                                    <th className="px-4 py-2 border-b">Tickets Sold</th>
                                </tr>
                            </thead>
                            <tbody>
                                {eventData.map((event) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-2 border-b">
                                            ₹ {event?.ticketPrice * event?.booked?.length}
                                        </td>
                                        <td className="px-4  py-2 border-b">
                                            {event?.booked?.length}
                                        </td>
