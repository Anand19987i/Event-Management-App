              ...
              <div className="loader">
                <div className="flex h-32 gap-4 p-5 fade-in-25 md:gap-6">
                  <div className="flex w-full max-w-3xl flex-col gap-4 rounded-lg pt-2">
                    <div className="h-5 w-10/12 origin-left animate-loading bg-[length:200%] rounded-sm bg-gradient-to-r from-purple-50 via-purple-600/60 to-blue-50 bg-2x opacity-0"></div>
                    <div className="h-5 w-full origin-left animate-loading bg-[length:200%] rounded-sm bg-gradient-to-r from-purple-500/60 via-slate-100 to-purple-500/60 bg-2x opacity-0"></div>
                    <div className="h-5 w-3/5 origin-left animate-loading bg-[length:200%] rounded-sm bg-gradient-to-r from-purple-50 via-purple-500/60 to-purple-50 bg-2x opacity-0"></div>
                  </div>
                </div>
              </div>
            ) : response ? (
              <div className="text-gray-800 space-y-4 bg-gray-200 p-4 rounded bg-opacity-40">
                <ReactMarkdown>{response}</ReactMarkdown>
                {/* <button
                  onClick={handleCopy}
                  className="flex absolute right-5 top-4 items-center gap-2 text-sm text-purple-600 bg-gray-100 px-3 py-1 rounded hover:bg-gray-200"
                >
                  <FaCopy />
                  {copied ? "Copied!" : "Copy"}
                </button> */}
